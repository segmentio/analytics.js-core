...
describe('each', function() {
  ...
  it('should work on objects (with no guarantee of iteration order)', function() {
    ...
    assert(
      identity.firstCall.calledWithExactly(elems[iter[0]], iter[0], elems)
    );
    assert(
      identity.secondCall.calledWithExactly(elems[iter[1]], iter[1], elems)
    );
    assert(
      identity.thirdCall.calledWithExactly(elems[iter[2]], iter[2], elems)
    );
  });
