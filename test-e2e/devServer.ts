...
(
    async() => {
        ...
        const resp = await fetch(`https://${config.local.originCDN}/analytics.js/v1/${config.local.testWriteKey}/analytics.js`)
        if (!resp.ok) {
            console.log(`Failed downloading analytics.js: ${resp.status} ${resp.statusText}`)
            process.exit(1)
        }
