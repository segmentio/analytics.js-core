...
Metrics.prototype._flush = function() {
  ...
  var payload = { series: this.queue.splice(0) };
  var url = 'https://' + this.host + '/m';

  // This endpoint does not support jsonp, so only proceed if the browser
  // supports xhr.
  if (send.type !== 'xhr') return;

  gzip.compress(payload, function(err, compressed, headers) {
    if (err) {
      debug(
        'failed to gzip data, failing back to uncompressed. input: %O, error: %O',
        payload,
        err
      );
    }
    ...
    send(url, compressed, headers, function(err, res) {
