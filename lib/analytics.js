...
Analytics.prototype.identify = function(id, traits, options, fn) {
  ...
  var newTraits = clone(traits) || {};
  var newOptions = clone(options) || {};

  if (plan.identify) {
    newOptions.blockedTraits = {};
    var defaultPlan = plan.identify.__default;
    this.log('applying identify plan - %o', plan.identify);

    for (var trait in newTraits) {
      if (newTraits.hasOwnProperty(trait)) {
        var traitPlan = plan.identify[trait];

        if (traitPlan) {
          this.log('identify plan for `%s` - %o', trait, traitPlan);
          if (traitPlan.enabled === false) {
            newOptions.blockedTraits[trait] = traits[trait];
            delete newTraits[trait];
          }
        } else if (defaultPlan) {
          this.log('identify default plan for `%s` - %o', trait, defaultPlan);
          if (defaultPlan.enabled === false) {
            newOptions.blockedTraits[trait] = traits[trait];
            delete newTraits[trait];
          }
        }
  ...
  // emit
  this.emit('identify', id, newTraits, newOptions);
  this._callback(fn);
  return this;
};
...
Analytics.prototype.group = function(id, traits, options, fn) {
  ...
  if (plan.group) {
    newOptions.blockedTraits = {};
    var defaultPlan = plan.group.__default;
    this.log('applying group plan - %o', plan.group);

    for (var trait in newTraits) {
      if (newTraits.hasOwnProperty(trait)) {
        var traitPlan = plan.group[trait];

        if (traitPlan) {
          this.log('group plan for `%s` - %o', trait, traitPlan);
          if (traitPlan.enabled === false) {
            newOptions.blockedTraits[trait] = traits[trait];
            delete newTraits[trait];
          }
        } else if (defaultPlan) {
          this.log('group default plan for `%s` - %o', trait, defaultPlan);
          if (defaultPlan.enabled === false) {
            newOptions.blockedTraits[trait] = traits[trait];
            delete newTraits[trait];
          }
  ...
  this.emit('group', id, newTraits, newOptions);
  this._callback(fn);
  return this;
};
...
Analytics.prototype.track = function(event, properties, options, fn) {
  ...
  var defaultPlan = trackPlans.__default;
  ...
  if (trackPlan) {
    this.log('track plan for `%s` - %o', event, trackPlan);
    ...
    this.log('track default plan for `%s` - %o', event, defaultPlan);
