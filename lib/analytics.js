  ...
  each(function(integration, name) {
    ...
    if (failedInitializations.indexOf(name) >= 0) {
      ...
      try {
        ...
        facadeCopy.obj = self._applyRouting(facadeCopy.obj, integration.name);
        if (facadeCopy.obj) {
          integration.invoke.call(integration, method, facadeCopy);
        } else {
          metrics.increment('analytics_js.event.filtered', {
            method: method,
            integration_name: integration.name
          });
        }
...
Analytics.prototype._applyRouting = function(event, destinationName) {
  if (!this.options.routingRules || !this.options.routingRules.length) {
    return event;
  }

  var store = new tsub.Store(this.options.routingRules);
  var rulesToApply = store.getRulesByDestinationName(destinationName);

  for (var i = 0; i < rulesToApply.length; i++) {
    var rule = rulesToApply[i];

    for (var j = 0; j < rule.matchers.length; j++) {
      // Each matcher has an array of transforms at the same index to apply if
      // that matcher is true.

      // e.g. rule = {
      //   ...rule,
      //   matchers: [{matcher1}, {matcher2}],
      //   transformers: [[{tf1.a}, {tf1.b}], [{tf2.a}, {tf2.b}, {tf2.c}]]
      // }
      var matcher = rule.matchers[j];
      var transformers = rule.transformers[j];

      if (tsub.matches(event, matcher)) {
        event = tsub.transform(event, transformers);

        // Early return if the entire facade event got dropped.
        if (event === null) {
          return null;
        }
      }
